...
impl DataManager {
    ...
    pub fn new<P: AsRef<Path>>(data_dir: P) -> Result<Self, CookbookError> {
        ...
        if !data_dir.exists() {
            ...
            return Err(CookbookError::DataDirError(format!(
                "Directory not found: {:?}",
                data_dir
            )));
        }
    ...
    fn load_ingredients(&mut self) -> Result<(), CookbookError> {
        ...
        if !ingredients_dir.exists() {
            return Err(CookbookError::DataDirError(format!(
                "Ingredients directory not found: {:?}",
                ingredients_dir
            )));
        }
    ...
    fn load_recipes(&mut self) -> Result<(), CookbookError> {
        ...
        let entries =
            fs::read_dir(&recipes_dir).map_err(|e| CookbookError::ListDirError(e.to_string()))?;
        ...
        for entry in entries {
            ...
            if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("md") {
                ...
                match Recipe::from_file(&path) {
                    ...
                    Err(e) => warn!("Failed to load recipe {:?}: {}", path, e), // If failed, log a warning
                }
            }
        }
    ...
    fn load_pantry(&mut self) -> Result<(), CookbookError> {
        ...
        if pantry_path.exists() {
            ...
            match Pantry::from_file(&pantry_path) {
                Ok(pantry) => {
                    info!(
                        "Successfully loaded pantry with {} items",
                        pantry.items.len()
                    );
                    for item in &pantry.items {
                        info!(
                            "Pantry item: {} (qty: {:?} {})",
                            item.ingredient, item.quantity, item.quantity_type
                        );
                    }
    ...
    fn load_kb_entries(&mut self) -> Result<(), CookbookError> {
        ...
        if !kb_dir.exists() {
            ...
            warn!("Knowledge Base directory not found: {:?} - continuing without KB entries", kb_dir);
            return Ok(());
        }

        // Read the contents of the kb directory
        let entries =
            std::fs::read_dir(&kb_dir).map_err(|e| CookbookError::ListDirError(e.to_string()))?;
        ...
        for entry in entries {
            ...
            if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("md") {
                match KnowledgeBaseEntry::from_file(&path) {
                    ...
                    Err(e) => warn!("Failed to load KB entry {:?}: {}", path, e),
                }
            }
        }
        Ok(())
    ...
    pub fn is_in_pantry(&self, ingredient_name: &str) -> bool {
        ...
        if let Some(pantry) = &self.pantry {
            ...
            let result = pantry
                .items
                .iter()
                .any(|item| item.ingredient == ingredient_name);
            debug!("'{}' in pantry: {}", ingredient_name, result);
            result
        } else {
            warn!(
                "No pantry loaded when checking for ingredient '{}'",
                ingredient_name
            );
            false
        }
    ...
    pub fn get_pantry_item(&self, ingredient_name: &str) -> Option<&PantryItem> {
        ...
        if let Some(pantry) = &self.pantry {
            ...
            let result = pantry
                .items
                .iter()
                .find(|item| item.ingredient == ingredient_name);
            if let Some(item) = result {
                debug!(
                    "Found pantry item: {} (qty: {:?} {})",
                    item.ingredient, item.quantity, item.quantity_type
                );
            } else {
            ...
            warn!(
                "No pantry loaded when getting pantry item for '{}'",
                ingredient_name
            );
            None
        }
        ...
        let mut results: Vec<&Recipe> = self
            ...
            .filter(|recipe| {
                recipe
                    .ingredients
                    .iter()
                    .any(|ing| ing.ingredient == ingredient_name)
            })
            .collect();
        ...
        self.get_all_ingredients()
            ...
            .filter(|ingredient| {
                ...
                let matches_category =
                    categories.is_empty() || categories.contains(&ingredient.category);
                // Match stock status
    ...
    pub fn get_pantry_items_by_category(
        &self,
    ) -> HashMap<String, Vec<(&Ingredient, Option<&PantryItem>)>> {
        let mut result = HashMap::new();
    ...
    pub fn get_kb_entry_for_ingredient(
        &self,
        ingredient_name: &str,
    ) -> Option<&KnowledgeBaseEntry> {
    ...
    pub fn update_pantry_item(
        ...
        let pantry = self
            .pantry
            .as_mut()
            .expect("Pantry should be loaded if not None");
        ...
        if !self.ingredients.contains_key(ingredient_name) {
            return Err(CookbookError::UpdateError(format!(
                "update_pantry_item: Ingredient '{}' does not exist",
                ingredient_name
            )));
        }
        ...
        let pantry_item_index = pantry
            .items
            .iter()
            .position(|item| item.ingredient == ingredient_name);
    ...
    pub fn update_ingredient(
        ...
        if !self.ingredients.contains_key(original_name) {
            return Err(CookbookError::UpdateError(format!(
                "update_ingredient: Ingredient '{}' does not exist",
                original_name
            )));
        }
        ...
        if original_name != new_ingredient.name
            && self.ingredients.contains_key(&new_ingredient.name)
        {
            return Err(CookbookError::UpdateError(format!(
                "update_ingredient: Cannot rename: ingredient '{}' already exists",
                new_ingredient.name
            )));
        }
        ...
        let new_path =
            ingredients_dir.join(format!("{}.yaml", new_ingredient.name.replace(" ", "_")));
        ...
        self.ingredients
            .insert(new_ingredient.name.clone(), new_ingredient.clone());
        ...
        if original_name != new_ingredient.name && old_path.exists() {
            fs::remove_file(old_path).map_err(|e| {
                CookbookError::WriteError(format!("Failed to remove old ingredient file: {}", e))
            })?;
        }
    ...
    pub fn update_recipe(
        ...
        if !self.recipes.iter().any(|r| r.title == original_title) {
            return Err(CookbookError::UpdateError(format!(
                "update_recipe: Recipe '{}' does not exist",
                original_title
            )));
        }

        // Check if the new title conflicts with an existing recipe (if title is changing)
        if original_title != new_recipe.title
            && self.recipes.iter().any(|r| r.title == new_recipe.title)
        {
            return Err(CookbookError::UpdateError(format!(
                "update_recipe: Cannot rename: recipe '{}' already exists",
                new_recipe.title
            )));
        }
        ...
        if original_title != new_recipe.title && old_path.exists() {
            fs::remove_file(old_path).map_err(|e| {
                CookbookError::WriteError(format!("Failed to remove old recipe file: {}", e))
            })?;
        }
    ...
    pub fn remove_from_pantry(&mut self, ingredient_name: &str) -> Result<bool, CookbookError> {
        if let Some(pantry) = self.pantry.as_mut() {
            ...
            pantry
                .items
                .retain(|item| item.ingredient != ingredient_name);
            let pantry_path = self.data_dir.join("pantry.yaml");
            pantry.to_file(pantry_path)?;
    ...
    pub fn update_ingredient_with_pantry(
        ...
        if remove_from_pantry {
            ...
            if let Some(pantry) = self.pantry.as_mut() {
                pantry
                    .items
                    .retain(|item| item.ingredient != new_ingredient.name);
                let pantry_path = self.data_dir.join("pantry.yaml");
                pantry.to_file(pantry_path)?;
            }
        } else {
    ...
    pub fn create_with_updated_ingredient(
        ...
        // Perform the update
        manager.update_ingredient_with_pantry(
            original_name,
            new_ingredient,
            quantity,
            quantity_type,
    ...
    pub fn are_all_ingredients_in_pantry(&self, recipe_title: &str) -> bool {
        ...
        if let Some(recipe) = self.get_recipe(recipe_title) {
            ...
            recipe
                .ingredients
                .iter()
                .all(|ingredient| self.is_in_pantry(&ingredient.ingredient))
        } else {
    ...
    pub fn delete_ingredient(&mut self, ingredient_name: &str) -> Result<bool, CookbookError> {
        ...
        if path.exists() {
            std::fs::remove_file(&path).map_err(|e| {
                CookbookError::WriteError(format!("Failed to remove ingredient file: {}", e))
            })?;
        }
        // Remove from pantry if present
        if let Some(pantry) = self.pantry.as_mut() {
    ...
    pub fn find_ingredient_by_name_or_translation(
        ...
        for ingredient in self.get_all_ingredients() {
            ...
            if let Some(translations) = &ingredient.translations {
                if let Some(forms) = translations.get(lang) {
                    if forms.one.to_lowercase() == query_lower
                        || forms.other.to_lowercase() == query_lower
                    {
                        return Some(ingredient);
                    }
                }
...
impl Recipe {
    ...
    pub fn all_ingredients_in_stock(&self, data_manager: &DataManager) -> bool {
        self.ingredients
            .iter()
            .all(|ingredient| data_manager.is_in_pantry(&ingredient.ingredient))
    }
}
